# Detailed Training and Prediction Process (Model B)

This document provides a deep dive into the inner workings of Model B (MobileNetV2 + LSTM), detailing how data flows from the dataset into the network, and how the model makes predictions.
...
2. **Spatial Feature Extraction (MobileNetV2):**
   * The `images_seq` is reshaped into a flat batch of images: `(batch_size * seq_len, 3, 224, 224)`.
   * It is passed through the pre-trained MobileNetV2 backbone (with the classifier head replaced by an Identity layer).
...
4. **Temporal Processing (LSTM):**
   * The combined sequence is fed into a Long Short-Term Memory (LSTM) layer with a hidden size of `128`.
   * The LSTM processes the sequential dependencies of the strawberry rotting over time, maintaining an internal cell state.
5. **Prediction (Regression Head):**
   * Only the output of the *last time step* from the LSTM `(batch_size, 128)` is utilized, as we are predicting the RUL for the current end-of-window state.
...
* **Model Checkpointing:** At the end of every epoch, validation loss is calculated. If the validation loss improves, the model weights are saved to `models/model_B/best_model.pth`.
...
1. **Initialization:** The script loads the `best_model.pth` checkpoint into the `StrawberryRULModelB` architecture and sets the model to evaluation mode (`model.eval()`).

# Model D: Training & Prediction Details
...
Input: (B, S, 3, 224, 224)
  ...
  → MobileNetV2.features(x)  →  (B*S, 1280, 7, 7)
```

MobileNetV2 uses inverted residual blocks with linear bottlenecks, making it more parameter-efficient than EfficientNet (~3.5M vs ~5.3M backbone parameters).
...
GRU uses a gating mechanism (reset + update gates) that is computationally simpler than LSTM's (input + forget + output gates + cell state) while often achieving comparable performance on shorter sequences.
...
| Model | Backbone Params | Total Params | Relative Speed |
|-------|----------------|--------------|----------------|
| Model A (EfficientNet+CBAM+GRU) | ~5.3M | ~5.7M | 1.0× (baseline) |
| Model B (MobileNetV2+CBAM+LSTM) | ~3.5M | ~3.9M | ~1.3× faster |
| Model C (EfficientNet+CBAM+LSTM) | ~5.3M | ~5.7M | ~0.95× |
| **Model D** (MobileNetV2+CBAM+GRU) | **~3.5M** | **~3.9M** | **~1.4× faster** |

Model D is the most efficient variant, suitable for resource-constrained environments.

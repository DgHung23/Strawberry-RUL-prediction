...
def build_basic_report(
    ...
    lines: list[str] = []
    lines.append('# EDA Starter Report')
    lines.append('')
    lines.append('This report is generated by the EDA starter notebook.')
    lines.append('')
    lines.append('## Paths')
    lines.append('')
    lines.append(f'- `MANIFEST_DIR`: `{report_dir.parent.parent.parent / "data" / "02_processed" / "manifests"}`')
    lines.append(f'- `REPORT_DIR`: `{report_dir}`')
    lines.append(f'- `GRAPH_DIR`: `{report_dir.parent.parent / "graphs" / "eda"}`')
    lines.append('')

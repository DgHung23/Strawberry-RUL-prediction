"""Tuned training script for Model D.
...
Outputs are written separately from the original Model D run:

- models/model_D_tuned/best_model.pth
- data/model_D_tuned_outputs/training_history.csv
- data/model_D_tuned_outputs/test_predictions.csv
- data/model_D_tuned_outputs/metrics.json
"""
...
def train(args: argparse.Namespace) -> dict[str, float]:
    ...
    models_dir = project_root / "models" / f"model_D_{run_name}"
    outputs_dir = project_root / "data" / f"model_D_{run_name}_outputs"
    ...
    metrics = {
        "model": f"Model_D_{run_name}_MobileNetV2_CBAM_GRU",
        "run_name": run_name,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train tuned Model D for strawberry RUL.")

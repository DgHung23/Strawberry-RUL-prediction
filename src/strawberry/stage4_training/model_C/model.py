...
class StrawberryRULModelC(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            fusion_mode: "early_concat" keeps the original architecture.
                "late_env_branch" encodes the image sequence first and fuses a
                compact environment branch before regression.
                "gated_env_branch" additionally uses environment context to gate
                the image temporal representation.
            temporal_pooling: "last" keeps the original last-timestep readout.
                "last_mean_max" concatenates last, mean, and max LSTM states.
            env_hidden_size: Hidden size of the late environment branch.
        """
        ...
        self.temporal_repr_dim = (
            rnn_hidden_size * 3
            if self.temporal_pooling == "last_mean_max"
            else rnn_hidden_size
        )

        if self.fusion_mode in {"late_env_branch", "gated_env_branch"}:
            env_summary_dim = self.env_dim * 3  # last, mean, delta

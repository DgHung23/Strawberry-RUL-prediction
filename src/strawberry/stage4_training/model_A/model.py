...
class StrawberryRULModel(nn.Module):
    ...
    def forward(
        ...
        if self.fusion_mode == "early_concat":
            temporal_input = torch.cat((spatial_features, env_seq), dim=2)
        else:
            temporal_input = spatial_features

        # ---- Step 3: Temporal modeling with GRU ----
        gru_out, _ = self.gru(temporal_input)  # (B, S, hidden_size)
        temporal_repr = self._pool_temporal(gru_out)

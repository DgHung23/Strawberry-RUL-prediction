"""Tuned training script for Model A.
...
Outputs are written separately from the original Model A run:

- models/model_A_tuned/best_model.pth
- data/model_A_tuned_outputs/training_history.csv
- data/model_A_tuned_outputs/test_predictions.csv
- data/model_A_tuned_outputs/metrics.json
"""
...
def train(args: argparse.Namespace) -> dict[str, float]:
    ...
    models_dir = project_root / "models" / f"model_A_{run_name}"
    outputs_dir = project_root / "data" / f"model_A_{run_name}_outputs"
    ...
    metrics = {
        "model": f"Model_A_{run_name}_EfficientNet-B0_CBAM_GRU",
        "run_name": run_name,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train tuned Model A for strawberry RUL.")

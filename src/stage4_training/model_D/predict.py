"""
Prediction script for Model D: MobileNetV2 + CBAM + GRU

Usage:
    python predict.py --image path/to/image.png --temp 22.5 --humidity 60.5
    python predict.py --image path/to/image.png --temp 22.5 --humidity 60.5 --checkpoint path/to/checkpoint.pth
"""
...
def predict(image_path, temp, humidity, checkpoint_path=None):
    ...
    if checkpoint_path is None:
        checkpoint_path = project_root / "models" / "model_D" / "best_model.pth"
    ...
    # Model expects a sequence: (batch_size, seq_len, C, H, W)
    # Use sequence of length 1 for single-image inference
    images_seq = img_tensor.unsqueeze(0).unsqueeze(0).to(device)  # (1, 1, 3, 224, 224)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Predict RUL from a single strawberry image (Model D).")
